      ...
      <div className="px-4 sm:px-6 py-2 sm:py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="relative notifications-container">
            ...
            {showNotifications && (
              <div className={`absolute right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden z-50 ${
                isMobile ? 'fixed top-14 left-4 right-4 w-auto max-w-none' : 'w-80 sm:w-96'
              }`}>
                <div className="p-3 sm:p-4 border-b border-gray-200 bg-gradient-to-r from-pink-50 to-purple-50 flex items-center justify-between">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-800">Notifications</h3>
                  <div className="flex items-center gap-2 sm:gap-3">
                    <span className="text-xs bg-pink-100 text-pink-700 px-2 py-1 rounded-full">{unreadCount} new</span>
                    {unreadCount > 0 && (
                      <button
                ...
                <div className="max-h-80 sm:max-h-96 overflow-y-auto">
                  {notifications.length > 0 ? (
                    notifications.map(n => {
                      const isNew = !n.read;
                      return (
                        <div key={n.id} className={`p-3 sm:p-4 border-b border-gray-100 hover:bg-gray-50 transition ${isNew ? 'bg-pink-50/30' : ''}`}>
                          <div className="flex gap-2 sm:gap-3">
                            {getNotificationIcon(n.type)}
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-1 sm:gap-2">
                                <div className="min-w-0">
                                  <p className="text-xs sm:text-sm font-medium text-gray-900 truncate">{n.title}</p>
                                  <p className="text-xs text-gray-600 mt-0.5 line-clamp-2">{n.message}</p>
                                </div>
                                {isNew && <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-pink-500 rounded-full flex-shrink-0 mt-1" />}
                              </div>
                              <div className="flex items-center gap-1 sm:gap-2 mt-2 flex-wrap">
                                <span className="text-xs text-gray-400">{n.time}</span>
                                {n.link && (
                                  <a href={n.link} className="text-xs text-pink-600 hover:text-pink-800 font-medium" onClick={e => e.stopPropagation()}>
                                    {isMobile ? 'View' : 'View details'} →
                                  </a>
                                )}
                                {isNew && (
                                  <button onClick={e => { e.stopPropagation(); markAsRead(n.id); }} className="text-xs text-gray-400 hover:text-gray-600 ml-auto">
                                    Mark read
                                  </button>
                                )}
                              </div>
                    ...
                    <div className="p-8 text-center text-gray-500"><p>No notifications</p></div>
                  )}
                </div>

                <div className="p-2 sm:p-3 border-t border-gray-200 bg-gray-50">
                  <button onClick={() => setShowNotifications(false)} className="w-full text-center text-xs text-gray-500 hover:text-gray-700">
                    Close
                  </button>
                </div>
              </div>
